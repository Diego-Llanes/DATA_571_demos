...
def main(args):
    ...
    def update(val):
        ...
        if args.loss == 'squared_error':
            grad_m = (2/args.N) * np.sum((true_x * m + b - true_y) * true_x)
            grad_b = (2/args.N) * np.sum(true_x * m + b - true_y)
        elif args.loss == 'absolute_error':
    ...
    def reset(val=None):
        ...
        # Reset learning rate slider to its initial value
        s_lr.set_val(0.01)
        ...
        # Recalculate Z for the new data
